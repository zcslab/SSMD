.onLoad <- function(libname, pkgname) {
  utils::data(IM_markers_20190302_mouse, package = pkgname, envir = parent.env(environment()))
  marker_stats1_uni <- SSMD::marker_stats1_uni
  assign("marker_stats1_uni", marker_stats1_uni, envir = parent.env(environment()))

  utils::data(Mouse_selected_core_markers, package = pkgname, envir = parent.env(environment()))
  tg_core_marker_set <- SSMD::tg_core_marker_set
  assign("tg_core_marker_set", tg_core_marker_set, envir = parent.env(environment()))
}
...
#'
#' @usage estimate.proportion <- function(data, lambda = 0.75)
#'
#' @param data input gene expression matrix. MGI gene symbol should be as their row names
#' @param lambda threshold of mean correlation to define rank-1 co-expression module
#'
#' @return An object of class is also invisibly returned. This is a list containing the following components:
...
estimate.proportion <- function(data, lambda = 0.75) {
  ...
  for (i in 1:length(intersect_marker1_choose)) {
    ...
    corr = stats::cor(t(data_c))

    clust = stats::hclust(stats::dist(corr))

    written_list = rep(0, dim(corr)[1])
    names(written_list) = row.names(corr)
    n = 1
    cut_value = 2
    t = stats::cutree(clust, k = cut_value)
    keep_k = vector("list", cut_value)
    marker_modules_cell_type = vector("list")
    marker_modules_length = 1

    while (cut_value < (length(clust$order) - 1)) {
      t = stats::cutree(clust, k = cut_value)
      for (k in 1:cut_value) {
        d = t[which(t == k)]
        # print(d)
        mean = mean(corr[names(d), names(d)])
        # print(mean)
        if (mean >= lambda & length(d) >= 5) {
  ...
  for (module_cell in unique(Stat_all$CT)) {
    ...
    bbb = aaa$ID[which((aaa$Core_overlap_number == max(unlist(aaa$Core_overlap_number))) | (aaa$Core_overlap_number >= 10) | ((aaa$Core_overlap_number >= 5) & (aaa$Core_overlap_rate >= 0.5)))]
